...
void Scattering::initOptions(IOptionsContainer *options, TrajectoryAnalysisSettings *settings) {
    static const char *const desc[] = {
            ...
            "[TT]-o[tt] writes resulting I(q)[PAR]",
            "[TT]-startq[tt] sets starting value of scattering vector q[PAR]",
            "[TT]-endq[tt] sets ending value of scattering vector q[PAR]",
            "[TT]-qspacing[tt] sets spacing between q points[PAR]",
            "[TT]-binwidth[tt] sets binwidth for P(r) histogramm",
            "[TT]-norm[tt] enables or disables nomalization of I(q) to I(0)[PAR]",
            "[TT]-scattering-type[tt] sets type of the computed curve (SANS or SAXS)[PAR]",
            "[TT]-mc[tt] enable Monte-Carlo mode for computing I(q)[PAR]",
            "[TT]-seed[tt] sets initial value for random seed for Monte-Carlo mode[PAR]",
            "[TT]-mc-coverage[tt] sets Monte-Carlo coverage respecting to Direct modes",
    ...
    options->addOption(
            DoubleOption("binwidth").store(&binwidth_).defaultValue(0.1).description("binwidth (nm) for P(r)"));
    ...
    options->addOption(
            IntegerOption("seed").store(&seed_).storeIsSet(&bSeedSet_).defaultValue(2023).description(
                    "random seed for Monte Carlo"));
    options->addOption(
            BooleanOption("norm").store(&bNormalize_).defaultValue(false).description(
    ...
    options->addOption(EnumOption<ScatterType>("scattering-type")
                               ...
                               .description("perform SAXS or SANS scattering"));
}
...
void Scattering::optionsFinished(TrajectoryAnalysisSettings * /*settings*/) {
    ...
    if ((bMonteCarloScattering_ && monteCarloCoverage_ <= 0) || monteCarloCoverage_ > 1) {
    ...
    for (size_t g = 0; g < sel_.size(); ++g) {
        ...
        switch (scatterEnum_) {
            ...
            case ScatterType::SAXS:
                ...
                break;
            default:
                GMX_THROW(InconsistentInputError("Unknown scattering type"));
        }
...
main(int argc, char *argv[]) {
    return gmx::TrajectoryAnalysisCommandLineRunner::runAsMain<Scattering>(argc, argv);
}
