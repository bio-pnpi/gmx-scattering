...
namespace gmx {
    ...
    void ComputeDebyeScattering::computeIntensity() {
        ...
        for (size_t qi = 0; qi < qValues_.size(); ++qi) {
            double intensityLocal_ = 0.F;
            if (qValues_[qi] == 0.0) {
                if (sfDepenOnQ_) {
                    for (auto histValue: sfQDependDistValues_[0]) {
                        intensityLocal_ += histValue;
                    }
                } else {
                    for (auto histValue: sfDistValues_) {
                        intensityLocal_ += histValue;
                    }
                }
            } else {
                for (size_t hi = 0; hi < maxHIndex_; ++hi) {
                    ...
                    if (sfDepenOnQ_) {
                        intensityLocal_ += sfQDependDistValues_[qi][hi] * sin(qDist) / qDist;
                    } else {
                        intensityLocal_ += sfDistValues_[hi] * sin(qDist) / qDist;
                    }
                }
            }
            intensity_[qi] = intensityLocal_;
        }
    }
